...
output_dir = r'F:\spontaneous_behavior\GitHub\The-hierarchical-behavioral-analysis-framework\Figure2_skeleton_kinematics(related to sFigure2)\stride'
...
def calculate_GapAndValue(distance_name,data,mv):
    ...
    #fig,ax = plt.subplots(nrows=1,ncols=1,figsize=(10,5),dpi=300,sharex=True)
    
    #ax.plot(data,label='original',c='black')
    #ax.plot(arr,label='smooth',c='red')
    ...
    #ax.scatter(peak1_time_index,peak1_value,c='green')
    #ax.scatter(valley1_time_index,-valley1_value,c='blue')
...
for key in list(dataset['video_index']):
    ...
    for mv in locomotion_list:#FeA_file['movement_label'].unique(): #,'left_turning','right_turning'
        ...
        #singleLocomotionFeA.loc['distance1'] = np.array(np.sqrt((Skeleton_file.loc['left_front_claw_x']-Skeleton_file.loc['left_hind_claw_x'])**2+(Skeleton_file.loc[:,'left_front_claw_y']-Skeleton_file.loc[:,'left_hind_claw_y'])**2))
        #average_stride = singleLocomotionFeA['distance1'].mean()
