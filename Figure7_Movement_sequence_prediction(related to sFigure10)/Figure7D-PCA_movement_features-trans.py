...
new_generate_data_MOTP_ASDP = r'F:\spontaneous_behavior\GitHub\The-hierarchical-behavioral-analysis-framework\Figure7_Movement_sequence_prediction(related to sFigure10)\predicted_movement_sequences\new_sequence-MOTP_ASDP'           #trans
new_generate_data_ASTP_MODP = r'F:\spontaneous_behavior\GitHub\The-hierarchical-behavioral-analysis-framework\Figure7_Movement_sequence_prediction(related to sFigure10)\predicted_movement_sequences\new_sequence-ASTP_MODP'
...
output_dir = r'F:\spontaneous_behavior\GitHub\The-hierarchical-behavioral-analysis-framework\Figure7_Movement_sequence_prediction(related to sFigure10)\PCA_results'
...
Movement_Label_path_predict_day_reversed = get_path2(new_generate_data_MOTP_ASDP,'Movement_Labels.csv')
Movement_Label_path_predict_night_reversed = get_path2(new_generate_data_ASTP_MODP,'Movement_Labels.csv')
...
for key in Movement_Label_path_predict_day_reversed.keys():
    video_index = key
    gender =  'None'
    ExperimentCondition =  dataset1_name.split('_')[0]
    LightingCondition =dataset1_name.split('_')[1]
    MoV_file = pd.read_csv(Movement_Label_path_predict_day_reversed[video_index])
    #location_file = pd.read_csv(Location_path2[video_index],usecols=['location'])
    #MoV_file = pd.concat([MoV_file,location_file],axis=1)
    MoV_matrix = extract_segment(MoV_file)
    MoV_matrix['group_id'] = 0
...
for key in Movement_Label_path_predict_night_reversed.keys():
    video_index = key
    gender =  'None'
    ExperimentCondition =  dataset2_name.split('_')[0]
    LightingCondition =dataset2_name.split('_')[1]
    MoV_file = pd.read_csv(Movement_Label_path_predict_night_reversed[video_index])
    #location_file = pd.read_csv(Location_path2[video_index],usecols=['location'])
    #MoV_file = pd.concat([MoV_file,location_file],axis=1)
    MoV_matrix = extract_segment(MoV_file)
    MoV_matrix['group_id'] = 1
...
colors = [dataset1_color, dataset1_color, dataset2_color,dataset2_color,'#42A5F5','#76FF03']   # stress_reversed 
