...
output_dir = r'F:\spontaneous_behavior\GitHub\The-hierarchical-behavioral-analysis-framework\Figure5_spatial_perference(related to sFigure8)\SG_index_in_difference_regions'
...
Coordinates_path = get_path(InputData_path_dir2,'coordinates_back_XY.csv')
...
def selectGroup_plotvariable(combine_df,plotvariable):
    ...
    plt.legend()
    plt.savefig(r'{}\comparision_of_{}.png'.format(output_dir,plotvariable),dpi=300,transparent=True,bbox_inches='tight')

def add_coordinates(FeA,Coor):
    
    for i in FeA.index:
        start = FeA.loc[i,'segBoundary_start']
        end = FeA.loc[i,'segBoundary_end']
        FeA.loc[i,'back_x'] = Coor.loc[start:end,'back_x'].mean()
        FeA.loc[i,'back_y'] = Coor.loc[start:end,'back_y'].mean()
    return(FeA)

def add_location(Mov_loc_data,boundary):
    Mov_loc_data_copy = Mov_loc_data.copy()
    Mov_loc_data_copy['location'] = 'perimeter'   
    Mov_loc_data_copy.loc[(Mov_loc_data_copy['back_x']>(250-boundary))&(Mov_loc_data_copy['back_x']<(250+boundary))&(Mov_loc_data_copy['back_y']>(250-boundary))&(Mov_loc_data_copy['back_y']<(250+boundary)),'location'] = 'center'
    Mov_loc_data_copy.loc[(Mov_loc_data_copy['back_x']<=(250-boundary))&(Mov_loc_data_copy['back_y']<=(250-boundary)),'location'] = 'corner'
    Mov_loc_data_copy.loc[(Mov_loc_data_copy['back_x']<=(250-boundary))&(Mov_loc_data_copy['back_y']>=(250+boundary)),'location'] = 'corner'
    Mov_loc_data_copy.loc[(Mov_loc_data_copy['back_x']>=(250+boundary))&(Mov_loc_data_copy['back_y']>=(250+boundary)),'location'] = 'corner'
    Mov_loc_data_copy.loc[(Mov_loc_data_copy['back_x']>=(250+boundary))&(Mov_loc_data_copy['back_y']<=(250-boundary)),'location'] = 'corner'
...
def plot_test(df):
    fig,ax = plt.subplots()
    ax.scatter(df['back_x'],df['back_y'])
    plt.show()


time_window1 = 10
time_window2 = 60
location = 'corner'         #center, perimeter, corner

output_dir = output_dir + '/{}-{}min_{}'.format(time_window1,time_window2,location)
if not os.path.exists(output_dir):                                    
    os.mkdir(output_dir)    
...
for index in dataset.index:
    ...
    MoV_file = add_location(MoV_file,175)
    MoV_file = MoV_file[MoV_file['location']==location]
    
    #MoV_file = MoV_file[((MoV_file['back_x']<(250-175))&(MoV_file['back_y']<(250-175)))|((MoV_file['back_x']<(250-175))&(MoV_file['back_y']>(250+175)))|((MoV_file['back_x']>(250+175))&(MoV_file['back_y']>(250+175)))|((MoV_file['back_x']>(250+175))&(MoV_file['back_y']<(250-175)))]
    MoV_file.reset_index(drop=True,inplace=True)
    #plot_test(MoV_file)
    Coor_data = pd.read_csv(Coordinates_path[video_index])
    ...
    FeA_file = add_coordinates(FeA_file,Coor_data)
    #FeA_file = FeA_file[((FeA_file['back_x']<(250-175))&(FeA_file['back_y']<(250-175)))|((FeA_file['back_x']<(250-175))&(FeA_file['back_y']>(250+175)))|((FeA_file['back_x']>(250+175))&(FeA_file['back_y']>(250+175)))|((FeA_file['back_x']>(250+175))&(FeA_file['back_y']<(250-175)))] 
    FeA_file = add_location(FeA_file,175)
    FeA_file = FeA_file[FeA_file['location']==location]
...
min_grooming_fra = np.min(grooming_df[grooming_df['movement_fraction']>0]['movement_fraction'])
min_grooming_fra = print(min_grooming_fra/10)

min_grooming_fre = np.min(grooming_df[grooming_df['movement_frequency']>0]['movement_frequency'])
min_grooming_fre = print(min_grooming_fre/10)

for i in grooming_df.index:
    if grooming_df.loc[i,'movement_fraction'] == 0:
        grooming_df.loc[i,'movement_fraction'] = 0.001
    if grooming_df.loc[i,'movement_frequency'] == 0:
        grooming_df.loc[i,'movement_frequency'] = 0.1
...
df_index['GS_fra_index'] = grooming_df['movement_fraction'].values/sniffing_df['movement_fraction'].values
df_index['GS_fre_index'] = grooming_df['movement_frequency'].values/sniffing_df['movement_frequency'].values
...
plt.savefig(r'{}\sniffing&groming_index_scatters.png'.format(output_dir),transparent=False,dpi=300,bbox_inches='tight')
...
plt.savefig(r'{}\sniffing&groming_MovementFraction_scatters.png'.format(output_dir),transparent=False,dpi=300,bbox_inches='tight')
...
plt.savefig(r'{}\sniffing&groming_MovementFrequency_scatters.png'.format(output_dir),transparent=False,dpi=300,bbox_inches='tight')
...
##################     The sniffing-to-grooming ratio  (violinplot) ##############################
...
ax.set_title('The sniffing-to-groming ratio (Movement Fraction)')
parts = ax.violinplot(
...
plt.savefig(r'{}\The_sniffing-to-groming_movementfraction_index.png'.format(output_dir),transparent=False,dpi=300)

fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(10, 5), sharey=True)
SG_fra_index_list = []
for group in dataset_order:
    SG_fra_index_list.append(df_index.loc[df_index['group']==group,'SG_fre_index'].values.tolist())
ax.set_title('The sniffing-to-groming ratio (Movement Frequency)')
parts = ax.violinplot(
        SG_fra_index_list, showmeans=False, showmedians=False,
        showextrema=False,widths=0.7)
for pc,color in zip(parts['bodies'],dataset_color.values()):
...
plt.savefig(r'{}\The_sniffing-to-groming_movementfrequency_index.png'.format(output_dir),transparent=False,dpi=300)

##################     The grooming-to-sniffing ratio  (violinplot) ##############################

fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(10, 5), sharey=True)
SG_fra_index_list = []
for group in dataset_order:
    SG_fra_index_list.append(df_index.loc[df_index['group']==group,'GS_fra_index'].values.tolist())
ax.set_title('The grooming-to-sniffing ratio (Movement Fraction)')
parts = ax.violinplot(
        SG_fra_index_list, showmeans=False, showmedians=False,
        showextrema=False,widths=0.7)
for pc,color in zip(parts['bodies'],dataset_color.values()):
...
plt.savefig(r'{}\The_grooming-to-snifffing_movementfraction_index.png'.format(output_dir),transparent=False,dpi=300)

fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(10, 5), sharey=True)
SG_fra_index_list = []
for group in dataset_order:
    SG_fra_index_list.append(df_index.loc[df_index['group']==group,'GS_fre_index'].values.tolist())
ax.set_title('The grooming-to-sniffing ratio (Movement Frequency)')
parts = ax.violinplot(
        SG_fra_index_list, showmeans=False, showmedians=False,
        showextrema=False,widths=0.7)
for pc,color in zip(parts['bodies'],dataset_color.values()):
...
plt.savefig(r'{}\The_grooming-to-snifffing_movementfrequency_index.png'.format(output_dir),transparent=False,dpi=300)


################################# random forest prediction ##################

X = df_index[['SG_fra_index','SG_fre_index']] #,'SG_fra_index','SG_fre_index']]
y = df_index['group_id']
...
sns.heatmap(pd.DataFrame(accumulate_matrix),annot=True, cmap="Blues",cbar=False,annot_kws={"fontsize":60,"fontfamily":'arial',"fontweight":'bold'})
classes = ['A','B','C','D','E']
tick_marks = np.arange(len(classes))
plt.xticks(tick_marks+0.5, classes,fontsize=60,fontfamily='arial',fontweight='bold')
plt.yticks(tick_marks+0.5, classes,fontsize=60,fontfamily='arial',fontweight='bold',rotation=0)

ax.tick_params(direction='out', length=6, width=2,pad =25)
...
#plt.axis('off')
plt.savefig(r'{}\RF_classification.png'.format(output_dir),transparent=False,dpi=300,bbox_inches='tight')
